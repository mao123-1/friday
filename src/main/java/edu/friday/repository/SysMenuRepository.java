...
public interface SysMenuRepository extends JpaRepository<SysMenu, Long>, SysMenuCustomRepository {
    String SELECT_DISTINCT = "select distinct m.menu_id, m.parent_id, m.menu_name, m.path, " +
            "m.component, m.visible, ifnull(m.perms,'') as perms, m.is_frame, m.menu_type, m.icon, m.order_num, " +
            "m.create_by, m.create_time, m.update_by, m.update_time, m.remark from sys_menu m ";
    ...
    @Query(value = SELECT_DISTINCT + JOIN_ROLE_MENU + JOIN_USER_ROLE + JOIN_ROLE + JOIN_USER +
            " where u.user_id = :userId and m.menu_type in ('M','C') and m.visible = 0 AND ro.status = 0 " + ORDER_BY, nativeQuery = true)
